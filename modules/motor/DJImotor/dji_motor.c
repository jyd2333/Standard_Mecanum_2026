...
extern float DM_4310_speed_target;
float speed =-1.0;
float kp_t=1.0;
float tff=-10.0;
...
static CANInstance sender_assignment[6] = {
    ...
    [4] = {.can_handle = &hcan2, .txconf.StdId = 0x200, .txconf.IDE = CAN_ID_STD, .txconf.RTR = CAN_RTR_DATA, .txconf.DLC = 0x08, .tx_buff = {0}},
    [5] = {.can_handle = &hcan2, .txconf.StdId = 0x1FF, .txconf.IDE = CAN_ID_STD, .txconf.RTR = CAN_RTR_DATA, .txconf.DLC = 0x08, .tx_buff = {0}},
};
#endif
...
static void DJIMotorLostCallback(void *motor_ptr)
{
    ...
    // HAL_CAN_Stop(motor->motor_can_instance->can_handle);
    // HAL_CAN_ActivateNotification(motor->motor_can_instance->can_handle, CAN_IT_RX_FIFO0_MSG_PENDING|CAN_IT_RX_FIFO1_MSG_PENDING);
    // HAL_CAN_Start(motor->motor_can_instance->can_handle);
    
    LOGWARNING("[dji_motor] Motor lost, can bus [%d] , id [%d]", can_bus, motor->motor_can_instance->tx_id);
}
// static uint8_t DM_ready=0;
// 电机初始化,返回一个电机实例
...
int index = 0;
    ...
    // CANTransmit(&sender_assignment[4], 1);
    // CANTransmit(&sender_assignment[4], 1);
