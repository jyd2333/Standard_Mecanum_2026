...
{
    pid->Dout = pid->Dout*pid->Derivative_LPF_RC  +
                pid->Last_Dout * (1-pid->Derivative_LPF_RC);
...
{
    pid->Output = pid->Output *pid->Output_LPF_RC +
                  pid->Last_Output *(1- pid->Output_LPF_RC);
    // pid->Output = pid->Output * pid->dt / (pid->Output_LPF_RC + pid->dt) +
    //               pid->Last_Output * pid->Output_LPF_RC / (pid->Output_LPF_RC + pid->dt);
}
//测量滤波
static void f_Measure_Filter(PIDInstance *pid)
{
    pid->Measure = pid->Measure *pid->Measure_LPF_RC +
                  pid->Last_Measure *(1- pid->Measure_LPF_RC);
}
// 输出限幅
...
{
    PIDInstance *pid = (PIDInstance *)zmalloc(sizeof(PIDInstance));
    pid->Kp          = config->Kp;
    pid->Ki          = config->Ki;
    pid->Kd          = config->Kd;
